...
//yourScheduel[i] = {date: --, workingon: --, timeStart: --, timeend: --, timespent: --}
 // example - {date:"03272000", workingon:"hack", timeStart:"6:00", timeend: "7:00", timespent:"1"}
  ...
  start = times[timeTracker].startTime;
      ...
      if(i!=0){
        ...
        if(i!=0){
          if(yourSchedule[i-1].timespent = 52){
            taskEnd = minToHour((hourToMin(start)+17));
            ...
            taskEnd = minToHour((hourToMin(start)+52));
        ...
        if(hourToMin(yourSchedule[i].timeend) > hourToMin(times[timeTracker].endTime)){
          timeTracker +=1;
        }
        i =i+1;
